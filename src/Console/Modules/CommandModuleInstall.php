...
class CommandModuleInstall extends Command
{
    use TraitModule;

	protected $signature = 'paagez:install-module {--module=} {--all}';
    ...
    public function allModules()
    {
        ...
        foreach ($modules as $key => $module) {
            ...
            if(!$module->installed && $module->can_install)
            {
        ...
        if($count_modules > 0)
        {
            ...
            $this->line("Successfully install <fg=yellow>{$success}</> modules...\n");
        }else{
            $this->line("\nFound <fg=yellow>{$count_modules}</>...\n");
        }
    }
    ...
    public function installModule($namespace)
    {
        ...
        $this->line("\nInstalling <fg=yellow>module\\$module->name</> on version <fg=yellow>module\\$module->version</>...\n");
