...
@push('meta')
<title>{{__('paagez.users')}} - {{config('paagez.app_name')}}</title>
@endpush
...
@section('contents')
	<div class="container mb-3">
		<ul class="d-flex p-2 w-100 align-items-center breadcrumb">
			...
			<li>
				<a href="{{ route(config('paagez.route_prefix').'.app.users.index') }}"><span>{{__('paagez.users')}}</span></a>
			</li>
		</ul>
	</div>
    <div class="container">
        <div class="row">
            @include("paagez::app.tab",['active'=>'users'])
            <div class="col-12">
                <div class="card p-3 shadow w-100 shadow border-0">
                    ...
                    <div class="d-flex align-items-center justify-content-between mb-3">
                        ...
                        <div class="d-flex">
                            <a href="{{ route(config('paagez.route_prefix').'.app.users.create') }}" class="btn btn-primary btn-sm ms-1"><i class="fa-solid fa-plus"></i> {{__("Create New")}}</a>
                        </div>
                    </div>
                        ...
                        <div class="position-absolute d-none bg-white rounded shadow w-50 z-3" id="filter">
                            <form class="p-3 border-bottom">
                                ...
                                <div class="mb-1">
                                    ...
                                    <input type="text" class="form-control form-control-sm" name="email" id="email" value="{{ request()->email }}">
                                </div>
                                <div class="mb-1">
                                    <label for="name">{{ __("paagez.roles") }}</label>
                                    <div class="d-flex flex-wrap">
                                        @foreach($roles as $role)
                                        <div class="form-check me-1">
                                          <input class="form-check-input" type="checkbox" value="{{$role->id}}" id="role_{{$role->id}}" name="roles[]" {{ (in_array($role->id,request()->roles??[])) ? 'checked' : '' }}>
                                ...
                                <div class="mt-3 d-flex">
                                    ...
                                    <a href="{{ route(config('paagez.route_prefix').'.app.users.index') }}" class="btn btn-secondary btn-sm">{{ __("Reset") }}</a>
                                </div>
                            </form>
                        </div>
                    </div>
                            ...
                            @if(count($datas) > 0)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <label class="badge bg-primary me-1">{{$role->name}} (<i>{{$role->guard_name}}</i>)</label>
                                        @endforeach
                                    </td>
                                    <td>
                                        @if($data->id != 1 && ($data->name != 'user_api' && $data->guard_name != 'api'))
                                        <a href="{{route(config('paagez.route_prefix').'.app.users.edit',[$data->id])}}" class="btn btn-secondary btn-sm" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{{ __("Edit") }}"><i class="fa-solid fa-edit"></i></a>
                                        ...
                                        <form method="POST" action="{{route(config('paagez.route_prefix').'.app.users.destroy',[$data->id])}}" id="delete-{{$data->id}}">
                                            @csrf
                                            <input type="hidden" name="_method" value="DELETE">
                                        </form>
                                        @else
                                        <i>{{__('paagez.users_default')}}</i>
                                        @endif
                                    </td>
                                </tr>
                                @endforeach
