...
@section("contents")
    <div class="container mb-3">
        <ul class="d-flex p-2 w-100 align-items-center breadcrumb">
            ...
            @if(!$data->id)
            <li>
                <a href="{{ route(config('paagez.route_prefix').'.app.users.create') }}"><span>{{__('Create')}} {{ __('paagez.users') }}</span></a>
            </li>
            @else
            <li>
                <a href="{{ route(config('paagez.route_prefix').'.app.users.edit',[$data->id]) }}"><span>{{__('Edit')}} {{ __('paagez.users') }}</span></a>
            </li>
            @endif
        </ul>
    </div>
    <div class="container">
        ...
        <div class="col-12">
            <div class="card p-3 shadow w-100 position-relative border-0">
                ...
                <x-alert-floating/>
                <form method="POST" action="{{ (!$data->id) ? route(config('paagez.route_prefix').".app.users.store") : route(config('paagez.route_prefix').".app.users.update",[$data->id]) }}" class="py-3">
                    @csrf
                    @if($data->id)
